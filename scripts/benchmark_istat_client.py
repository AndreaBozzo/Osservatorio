...
class PerformanceBenchmark:
    ...
    def benchmark_single_dataset_fetch(self, iterations: int = 3):
        ...
        for i in range(iterations):
            with self.timer("production_single_fetch"):
                try:
                    result = self.production_client.fetch_dataset(dataset_id, include_data=True)
    ...
    async def benchmark_batch_processing(self):
        ...
        print(f"⚡ Benchmarking batch processing ({len(self.test_datasets)} datasets)...")
        ...
        start_time = time.time()
        try:
            batch_result = await self.production_client.fetch_dataset_batch(self.test_datasets)
        ...
        print(f"  Legacy (sequential): {legacy_time:.3f}s, {legacy_successful}/{len(self.test_datasets)} successful")
        print(f"  Production (concurrent): {production_time:.3f}s, {production_successful}/{len(self.test_datasets)} successful")
    ...
    def benchmark_quality_validation(self, iterations: int = 2):
        ...
        for i in range(iterations):
            with self.timer("legacy_quality"):
                try:
                    quality = self.legacy_tester.validate_data_quality(dataset_id, sample_size=500)
        ...
        for i in range(iterations):
            with self.timer("production_quality"):
                try:
                    quality = self.production_client.fetch_with_quality_validation(dataset_id)
        ...
        for i in range(iterations):
            with self.timer("production_storage"):
                try:
                    ...
                    sync_result = self.production_client.sync_to_repository(dataset_result)
    ...
    def benchmark_error_resilience(self):
        ...
        print(f"  Production client: Circuit breaker activated in {circuit_breaker_time:.3f}s")
    ...
    def generate_performance_report(self):
        ...
        if "legacy_batch" in self.results and "production_batch" in self.results:
            batch_improvement = self.results["legacy_batch"][0] / self.results["production_batch"][0]
            print(f"Batch Processing:        {batch_improvement:.1f}x faster (concurrent vs sequential)")
            print()
        ...
        for operation, times in self.results.items():
            if times:
                ...
                print(f"{operation:25} {avg_time:.3f}s avg (min: {min_time:.3f}s, max: {max_time:.3f}s)")
