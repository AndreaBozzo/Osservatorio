...
class PowerBISetupWizard:
    ...
    def run_setup(self):
        ...
        print("=" * 50)
        print("Questa guida ti aiuterà a configurare l'integrazione PowerBI con Azure AD")
        print()
    ...
    def guide_azure_ad_setup(self):
        ...
        print("   c) Tipi di account supportati: 'Account in questa directory organizzativa'")
        ...
        self.config['client_id'] = input("   Inserisci Application (client) ID: ").strip()
        ...
        self.config['client_secret'] = input("   Inserisci Client Secret (valore): ").strip()
    ...
    def test_connection(self):
        ...
        try:
            ...
            if test_result['workspaces_accessible']:
                self.log_step(f"✅ Workspace accessibili: {test_result['workspace_count']}", "SUCCESS")
            else:
                self.log_step("❌ Nessun workspace accessibile", "WARNING")
            
            if test_result['datasets_accessible']:
                self.log_step(f"✅ Dataset accessibili: {test_result['dataset_count']}", "SUCCESS")
            else:
                self.log_step("ℹ️ Nessun dataset nel workspace (normale per prima configurazione)", "INFO")
            
            # Salva risultati test
            test_file = f"powerbi_setup_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def save_configuration(self):
        ...
        filtered_content.append(f'POWERBI_CLIENT_SECRET={self.config["client_secret"]}\n')
    ...
    def show_summary(self):
        ...
        with open(log_file, 'w', encoding='utf-8') as f:
            ...
            f.write("=" * 20 + "\n\n")
            f.write(f"Setup completato: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
