...
def validate_step1_2():
    ...
    if powerbi_dir.exists():
        ...
        if conversion_files:
            add_check("File Conversione", "PASS", f"Trovati {len(conversion_files)} file di conversione")
        else:
    ...
    try:
        ...
        add_check("MSAL Library", "FAIL", "MSAL non installato - eseguire: pip install msal")
    
    try:
        import pyarrow
        add_check("PyArrow Library", "PASS", "PyArrow installato per formato Parquet")
    except ImportError:
        add_check("PyArrow Library", "FAIL", "PyArrow non installato - eseguire: pip install pyarrow")
    ...
    if env_file.exists():
        ...
        add_check("File .env", "WARN", "File .env non esistente - verrà creato durante setup")
    ...
    for var in powerbi_vars:
        ...
        if value:
            ...
            add_check(f"Variabile {var}", "WARN", "Non configurata - da configurare durante setup")
    ...
    warning_checks = len([c for c in validation_results['checks'] if c['status'] == 'WARN'])
    ...
    print(f"Pronto per esecuzione: {'✅ SÌ' if validation_results['ready_for_execution'] else '❌ NO'}")
