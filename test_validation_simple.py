    ...
    for dataset_id, should_pass, description in test_cases:
        try:
            ...
            if should_pass:
                ...
                print(f"❌ FAIL: {description} - '{dataset_id}' should have failed but passed")
                failed += 1
        except HTTPException as e:
            if not should_pass:
                error_msg = e.detail.get('error', 'Unknown error') if isinstance(e.detail, dict) else str(e.detail)
                print(f"✅ EXPECTED FAIL: {description} - '{dataset_id}' -> {error_msg}")
                passed += 1
            else:
                print(f"❌ UNEXPECTED FAIL: {description} - '{dataset_id}' should have passed")
    ...
    for invalid_id, error_type in error_test_cases:
        try:
            ...
            if isinstance(e.detail, dict):
                print(f"🗨️  {error_type.upper()}: {e.detail.get('error', 'No error field')}")
