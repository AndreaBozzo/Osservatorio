...
class IstatXMLToPowerBIConverter:
    ...
    def _load_datasets_config(self) -> Dict:
        ...
        config_files = [f for f in os.listdir('.') if f.startswith('tableau_istat_datasets_') and f.endswith('.json')]
        
        if not config_files:
            logger.warning("File configurazione non trovato, creazione configurazione di esempio...")
    ...
    def convert_all_datasets(self) -> None:
        ...
        logger.info(f"Conversione di {len(self.datasets_config['datasets'])} dataset per PowerBI...")
    ...
    def _convert_single_dataset(self, dataset: Dict) -> None:
        ...
        if not xml_file:
            logger.warning(f"File XML non trovato per {dataflow_id}, creazione dati di esempio...")
            ...
            try:
                ...
                if not observations:
                    logger.warning(f"Nessuna osservazione trovata nel file XML per {dataflow_id}")
                    df_clean = self._create_sample_data(dataflow_id, name, category)
                else:
                    df = pd.DataFrame(observations)
                    df_clean = self._clean_dataframe_for_powerbi(df, category, dataflow_id)
        ...
        logger.info(f"Convertito {dataflow_id}: {len(df_clean)} righe, {len(df_clean.columns)} colonne")
    ...
    def _clean_dataframe_for_powerbi(self, df: pd.DataFrame, category: str, dataflow_id: str) -> pd.DataFrame:
        """Pulisce DataFrame specificamente per PowerBI."""
        df_clean = df.copy()
        
        # Standardizza nomi colonne per PowerBI (no spazi, caratteri speciali)
        df_clean.columns = [self._clean_column_name_for_powerbi(col) for col in df_clean.columns]
        
        # Gestisci date/tempo
        time_columns = [col for col in df_clean.columns if 'time' in col.lower() or 'date' in col.lower()]
        for time_col in time_columns:
            df_clean[time_col] = self._standardize_time_column(df_clean[time_col])
        
        # Gestisci valori numerici
        value_columns = [col for col in df_clean.columns if 'value' in col.lower() or 'obs' in col.lower()]
        ...
        available_cols = [col for col in priority_cols if col in df_clean.columns] + other_cols
    ...
    def _standardize_time_column(self, time_series: pd.Series) -> pd.Series:
        ...
        def parse_time(time_val):
            ...
            patterns = [
                ...
                (r'^(\d{4})-Q(\d)$', lambda m: f"{m.group(1)}-{str(int(m.group(2))*3).zfill(2)}-01"),
    ...
    def _create_sample_data(self, dataflow_id: str, name: str, category: str) -> pd.DataFrame:
    ...
    def _save_powerbi_formats(self, df: pd.DataFrame, dataflow_id: str, name: str, category: str) -> Dict[str, str]:
    ...
    def _generate_powerbi_summary(self) -> None:
        ...
        summary = {
            ...
            'success_rate': f"{(len(successful)/len(self.conversion_results)*100):.1f}%" if self.conversion_results else "0%",
        ...
        logger.info(f"Conversione completata: {len(successful)}/{len(self.conversion_results)} dataset convertiti")
    
    def _generate_powerbi_guide(self, successful_datasets: List[Dict], timestamp: str) -> None:
        """Genera guida specifica per PowerBI."""
        guide_path = self.powerbi_output_dir / f"powerbi_integration_guide_{timestamp}.md"
