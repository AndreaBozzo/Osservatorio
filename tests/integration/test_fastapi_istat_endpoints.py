...
class TestIstatStatusEndpoint:
    ...
    def test_status_endpoint_success(self, client, auth_headers):
        ...
        with patch("src.api.dependencies.get_istat_client") as mock_get_client, patch(
...
class TestIstatDataflowsEndpoint:
    ...
    def test_dataflows_endpoint_success(self, client, auth_headers):
        ...
        with patch(
            "src.api.fastapi_app.get_production_istat_client"
        ) as mock_get_client, patch(
            "src.api.dependencies.verify_jwt_token"
        ) as mock_verify_jwt:
...
class TestIstatSyncEndpoint:
    ...
    def test_sync_endpoint_success(self, client, auth_headers):
        ...
        with patch(
            ...
            data = response.json()
            assert data["dataset_id"] == "101_1015"
            assert data["records_synced"] == 1500
            assert data["metadata_updated"] is True
            assert data["sync_time"] == 2.5
...
class TestIstatEndpointsIntegration:
    ...
    def test_complete_istat_workflow(self, client, auth_headers):
        ...
        with patch(
            ...
            # 1. Check status
            status_response = client.get("/api/istat/status")
            ...
            assert sync_response.json()["records_synced"] == 1000
