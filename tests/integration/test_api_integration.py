...
class TestAPIIntegration:
    ...
    def test_istat_api_connectivity(self, mock_requests_session):
        ...
        mock_requests_session.get.return_value.content = b'<?xml version="1.0"?><test>data</test>'
    ...
    def test_dataflow_discovery_integration(self, mock_requests_session, sample_dataflow_xml):
    ...
    def test_api_error_handling(self, mock_requests_session):
        ...
        for status_code, error_message in error_scenarios:
            ...
            mock_requests_session.get.return_value.content = error_message.encode('utf-8')
    ...
    def test_api_timeout_handling(self, mock_requests_session):
        ...
        mock_requests_session.get.side_effect = requests.exceptions.Timeout("Request timeout")
    ...
    def test_api_rate_limiting(self, mock_requests_session):
        ...
        priority_datasets = [
            {'id': f'dataset_{i}', 'name': f'Dataset {i}', 'category': 'test', 'relevance_score': 10}
            for i in range(3)
        ]
    ...
    def test_comprehensive_integration_workflow(self, mock_requests_session, sample_dataflow_xml, sample_xml_data):
        ...
        mock_requests_session.get.return_value.status_code = 200
        mock_requests_session.get.return_value.elapsed.total_seconds.return_value = 1.0
        mock_requests_session.get.return_value.content = b'<?xml version="1.0"?><test>ok</test>'
        ...
        tableau_ready = analyzer.create_tableau_ready_dataset_list(testing_result['dataset_tests'])
        
        # Should have some successful conversions
        successful_datasets = [ds for ds in testing_result['dataset_tests'] 
        ...
        successful_connections = sum(1 for conn in full_report['connectivity'] if conn.get('success'))
        successful_tests = sum(1 for test in full_report['dataset_tests'] 
    ...
    def test_api_response_validation(self, mock_requests_session):
        ...
        for response_content, should_be_valid in test_cases:
            mock_requests_session.get.return_value.status_code = 200
            mock_requests_session.get.return_value.content = response_content.encode('utf-8')
