...
class TestTableauIstatScraper:
    ...
    def test_suggest_use_case(self):
        ...
        assert "CSV ISTAT" in scraper._suggest_use_case("Google Sheets")  # returns Google Sheets use case
        assert "grandi dataset" in scraper._suggest_use_case("BigQuery")  # returns BigQuery use case
        assert "Condivisione file" in scraper._suggest_use_case("Box")  # returns Box use case
        # Note: dropbox returns the same as box due to substring matching - both contain "box"
        assert "Condivisione file" in scraper._suggest_use_case("dropbox")  # returns Box use case (substring match)
        assert "Microsoft Office" in scraper._suggest_use_case("onedrive")  # returns OneDrive use case (lowercase)
        assert "generico" in scraper._suggest_use_case("Unknown Connector")  # fallback case
