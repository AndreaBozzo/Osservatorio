...
class TestIstatAPITester:
    ...
    def test_run_comprehensive_test(self, mock_session_class):
        ...
        with patch.object(tester, 'test_api_connectivity', return_value=mock_connectivity_result) as mock_connectivity, \
             patch.object(tester, 'discover_available_datasets', return_value=[]) as mock_discover, \
             patch.object(tester, 'test_popular_datasets', return_value=[]) as mock_popular, \
             patch.object(tester, 'validate_data_quality', return_value=True) as mock_quality, \
             patch.object(tester, 'create_data_preview_visualization', return_value=True) as mock_viz, \
             patch.object(tester, 'generate_final_report', return_value=True) as mock_report:
