...
class TestConfig:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(Config, 'BASE_DIR', temp_path):
                with patch.object(Config, 'DATA_DIR', temp_path / "data"):
                    with patch.object(Config, 'RAW_DATA_DIR', temp_path / "data" / "raw"):
                        with patch.object(Config, 'PROCESSED_DATA_DIR', temp_path / "data" / "processed"):
                            with patch.object(Config, 'CACHE_DIR', temp_path / "data" / "cache"):
                                with patch.object(Config, 'LOGS_DIR', temp_path / "logs"):
                                    Config.ensure_directories()
                                    
                                    # Check directories were created
                                    assert (temp_path / "data" / "raw" / "istat").exists()
                                    assert (temp_path / "data" / "raw" / "xml").exists()
                                    assert (temp_path / "data" / "processed" / "tableau").exists()
                                    assert (temp_path / "data" / "processed" / "powerbi").exists()
                                    assert (temp_path / "data" / "cache").exists()
                                    assert (temp_path / "logs").exists()
                                    
                                    # Check .gitkeep files were created
                                    assert (temp_path / "data" / "raw" / "istat" / ".gitkeep").exists()
                                    assert (temp_path / "data" / "cache" / ".gitkeep").exists()
    ...
    def test_powerbi_config_optional(self):
        ...
        assert Config.POWERBI_CLIENT_SECRET is None or Config.POWERBI_CLIENT_SECRET == ""
