...
class TestDataConversion:
    ...
    def test_dataframe_to_csv_conversion(self):
        ...
        try:
            ...
            assert list(df_read.columns) == ['territorio', 'anno', 'valore', 'unita_misura']
    ...
    def test_data_validation_and_cleaning(self):
        ...
        # Test data cleaning
        df_clean = df.dropna()
        assert len(df_clean) == 2  # Two rows remain (row with 'invalid' is kept by dropna)
    ...
    def test_conversion_summary_generation(self):
        ...
        total_datasets = len(conversion_results['successful_datasets']) + len(conversion_results['failed_datasets'])
        success_rate = (len(conversion_results['successful_datasets']) / total_datasets) * 100
