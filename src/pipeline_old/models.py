...
class QualityScore:
    ...
    def __post_init__(self):
        ...
        self.overall_score = (
            self.completeness + self.consistency + self.accuracy + self.timeliness
        ) / 4.0
...
class PipelineConfig(BaseModel):
    ...
    min_quality_score: float = Field(
        default=60.0, description="Minimum acceptable quality score"
    )
    enable_quality_checks: bool = Field(
        default=True, description="Enable quality validation"
    )
    fail_on_quality: bool = Field(
        default=False, description="Fail pipeline on low quality"
    )

    # Output settings
    store_raw_data: bool = Field(default=True, description="Store original SDMX data")
    store_analytics: bool = Field(
        default=True, description="Store processed analytics data"
    )
    ...
    retry_delay: float = Field(
        default=2.0, description="Delay between retries (seconds)"
    )
...
class BatchResult(BaseModel):
    ...
    completed_datasets: int = Field(
        default=0, description="Successfully completed datasets"
    )
