...
class IstatDataflowAnalyzer:
    def __init__(self):
        ...
        self.category_keywords = {
            'popolazione': {
                'keywords': ['popolazione', 'popul', 'residente', 'demografic', 'demo', 'nascite', 'morti', 'stranieri'],
                'priority': 10
            },
            'economia': {
                'keywords': ['pil', 'gdp', 'economia', 'economic', 'inflazione', 'prezzi', 'price', 'reddito', 'income'],
                'priority': 9
            },
            'lavoro': {
                'keywords': ['lavoro', 'occupazione', 'disoccupazione', 'employment', 'unemploy', 'forze_lavoro'],
                'priority': 8
            },
            'territorio': {
                'keywords': ['regione', 'provincia', 'comune', 'territorial', 'geographic'],
                'priority': 7
            },
            'istruzione': {
    ...
    def test_priority_dataflows(self, top_dataflows, max_tests=15):
        ...
        priority_order = ['popolazione', 'economia', 'lavoro', 'territorio', 'istruzione', 'salute']
    ...
    def _test_single_dataflow(self, dataflow):
        ...
        try:
            ...
            result['tests']['data_access'] = {
                ...
                'size_bytes': len(data_response.content) if data_response.status_code == 200 else 0
            }
            ...
            if data_response.status_code == 200:
                ...
                print(f"    ❌ Dati non accessibili (Status: {data_response.status_code})")
        ...
        for df in tested_dataflows:
            if df['tests'].get('data_access', {}).get('success', False):
                tableau_entry = {
                    ...
                    'data_size_mb': df['tests']['data_access']['size_bytes'] / (1024*1024),
                    ...
                    'refresh_frequency': self._suggest_refresh_frequency(df['category']),
    ...
    def _calculate_priority(self, dataflow):
        ...
        size_bonus = min(5, dataflow['tests']['data_access']['size_bytes'] / (1024*1024) / 10)
...
def main():
    ...
    if not categorized:
        print("❌ Impossibile analizzare dataflow. Verifica che dataflow_response.xml esista.")
        return
