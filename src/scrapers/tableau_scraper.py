...
class TableauIstatScraper:
    ...
    def setup_authentication(self, username, password):
        ...
        auth_payload = {
            'credentials': {
                ...
                'password': password,
                'site': {
                    'contentUrl': self.config['result']['site']['urlName']
                }
            }
        }
    ...
    def _analyze_server_capabilities(self):
        ...
        capabilities = {
            'web_authoring': self.config['result']['site']['settings']['webEditingEnabled'],
            'flows_enabled': self.config['result']['site']['settings']['flowAutoSaveEnabled'],
            'data_management': self.config['result']['site']['dataManagementEnabled'],
            'scheduling': self.config['result']['site']['runNowEnabled'],
            'extract_encryption': self.config['result']['site']['extractEncryptionMode']
        }
    ...
    def _get_recommended_connectors(self):
        ...
        for setting in oauth_settings:
            ...
            if any(keyword in connector_name.lower() for keyword in [
                'google sheets', 'bigquery', 'box', 'dropbox', 'onedrive'
            ]):
                recommended.append({
                    'name': connector_name,
                    'type': setting['type'],
