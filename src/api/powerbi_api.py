...
class PowerBIAPIClient:
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            if "access_token" in result:
                ...
                logger.error(f"Errore autenticazione: {result.get('error_description', 'Unknown error')}")
                return False
    ...
    def get_datasets(self, workspace_id: Optional[str] = None) -> List[Dict]:
        ...
        try:
            response = self.session.get(f"{self.base_url}/groups/{workspace_id}/datasets")
            response.raise_for_status()
            
            datasets = response.json().get("value", [])
            logger.info(f"Recuperati {len(datasets)} dataset dal workspace {workspace_id}")
            return datasets
    ...
    def get_reports(self, workspace_id: Optional[str] = None) -> List[Dict]:
        ...
        try:
            response = self.session.get(f"{self.base_url}/groups/{workspace_id}/reports")
            response.raise_for_status()
            
            reports = response.json().get("value", [])
            logger.info(f"Recuperati {len(reports)} report dal workspace {workspace_id}")
            return reports
    ...
    def create_dataset(self, dataset_definition: Dict, workspace_id: Optional[str] = None) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(f"Dataset creato: {dataset.get('name')} (ID: {dataset.get('id')})")
            return dataset
    ...
    def push_data_to_dataset(self, dataset_id: str, table_name: str, data: List[Dict],
        ...
        try:
            ...
            logger.info(f"Dati inviati al dataset {dataset_id}, tabella {table_name}: {len(data)} righe")
            return True
    ...
    def get_dataset_refresh_history(self, dataset_id: str, workspace_id: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Recuperata cronologia refresh per dataset {dataset_id}: {len(history)} elementi")
            return history
    ...
    def upload_pbix_file(self, file_path: str, dataset_name: str, 
        ...
        try:
            ...
            logger.info(f"File PBIX caricato: {dataset_name} (Import ID: {result.get('id')})")
            return result
...
def main():
    ...
    if not all([Config.POWERBI_CLIENT_ID, Config.POWERBI_CLIENT_SECRET, Config.POWERBI_TENANT_ID]):
        print("❌ Configurazione PowerBI incompleta!")
        print("Imposta le variabili d'ambiente: POWERBI_CLIENT_ID, POWERBI_CLIENT_SECRET, POWERBI_TENANT_ID")
        return
    ...
    print(f"Workspace accessibili: {'✅' if test_result['workspaces_accessible'] else '❌'}")
