...
class TableauServerAnalyzer:
    ...
    def get_available_connectors(self):
        ...
        for setting in oauth_settings:
            ...
            connectors.append({
                ...
                'supports_custom_domain': setting.get('supportsCustomDomain', False)
            })
    ...
    def find_data_sources(self):
        ...
        useful_connectors = [
            ...
            if any(keyword in conn['name'].lower() for keyword in [
                'google', 'bigquery', 'analytics', 'sheets', 'salesforce', 
                'box', 'dropbox', 'onedrive', 'azure', 'amazon'
            ])
        ]
    ...
    def generate_scraping_strategy(self):
        ...
        for conn in connectors:
            if 'google' in conn['name'].lower():
                ...
                strategy['direct_api'].append(f"Connessione diretta a BigQuery per grandi dataset")
