...
from src.api.dependencies import validate_dataset_id, get_dataset_id_suggestions
...
class TestEnhancedValidateDatasetId:
    ...
    def test_invalid_characters(self):
        ...
        for dataset_id, expected_chars in invalid_cases:
            ...
            print(f"✅ Invalid chars detected: '{dataset_id}' -> {error_detail['message']}")
    ...
    def test_leading_trailing_separators(self):
        ...
        for dataset_id in invalid_cases:
            ...
            error_detail = exc_info.value.detail
            assert "cannot start" in error_detail["message"].lower() or "cannot end" in error_detail["message"].lower()
            assert "corrected_suggestion" in error_detail
            print(f"✅ Leading/trailing separator: '{dataset_id}' -> {error_detail['corrected_suggestion']}")
    ...
    def test_consecutive_separators(self):
        ...
        for dataset_id in invalid_cases:
            ...
            assert "corrected_suggestion" in error_detail
            print(f"✅ Consecutive separators: '{dataset_id}' -> {error_detail['corrected_suggestion']}")
...
class TestDatasetIdSuggestions:
    ...
    def test_cleaned_suggestion_included(self):
        ...
        for invalid_id, expected_clean in test_cases:
            ...
            assert suggestions[0] == expected_clean, f"Expected '{expected_clean}' but got '{suggestions[0]}' for input '{invalid_id}'"
    ...
    for dataset_id, should_pass in test_batch:
        try:
            ...
            if not should_pass:
                ...
                print(f"❌ UNEXPECTED FAIL: '{dataset_id}' should have passed -> {e.detail}")
                results["failed"] += 1
