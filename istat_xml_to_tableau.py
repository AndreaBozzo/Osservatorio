...
class IstatXMLtoTableauConverter:
    ...
    def _load_datasets_config(self):
        ...
        if not config_files:
            print("⚠️  File configurazione non trovato. Creazione configurazione di esempio...")
    ...
    def _convert_single_dataset(self, dataset, output_dir):
        ...
        output_files = self._save_multiple_formats(df_clean, dataflow_id, name, category, output_dir)
        ...
        print(f"   ✅ Convertito: {len(df_clean)} righe, {len(df_clean.columns)} colonne")
    ...
    def _standardize_time_column(self, time_series):
        ...
        def parse_time(time_val):
            ...
            patterns = [
                ...
                (r'^(\d{4})-(\d{1,2})$', lambda m: f"{m.group(1)}-{m.group(2).zfill(2)}-01"),  # 2024-03
                (r'^(\d{4})-Q(\d)$', lambda m: f"{m.group(1)}-{str(int(m.group(2))*3).zfill(2)}-01"),  # 2024-Q1
    ...
    def _generate_conversion_summary(self, output_dir):
        ...
        summary = {
            ...
            'files_generated': {
                'csv_files': len([f for r in successful for f in r.get('output_files', {}).values() if f.endswith('.csv')]),
                'excel_files': len([f for r in successful for f in r.get('output_files', {}).values() if f.endswith('.xlsx')]),
                'json_files': len([f for r in successful for f in r.get('output_files', {}).values() if f.endswith('.json')])
            }
        }
        ...
        print("=" * 60)
        print(f"✅ Dataset convertiti con successo: {len(successful)}/{len(self.conversion_results)}")
        ...
        if failed:
            print(f"\n❌ DATASET FALLITI:")
            for fail in failed:
                print(f"  • {fail['dataflow_id']}: {fail.get('error', 'Unknown error')}")
